document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * Thử fetch từ Netlify Functions, nếu không có function (404 Function not found) thì fallback sang static /data/*.json
     */
    async function fetchFromApiOrPublic(sheetNameNormalized, sheetNameForAPI) {
        // Thử gọi Netlify Function trước
        try {
            const response = await fetch(API_URLS[sheetNameNormalized]);
            if (!response.ok) {
                const errorText = await response.text();
                // Nếu function không tồn tại, fallback sang static JSON
                if (response.status === 404 && /Function not found/i.test(errorText)) {
                    const fallbackUrl = `/data/${sheetNameForAPI}.json`;
                    const fallbackRes = await fetch(fallbackUrl);
                    if (!fallbackRes.ok) {
                        const fallbackErr = await fallbackRes.text();
                        throw new Error(`Fallback error ${fallbackRes.status} for ${sheetNameForAPI}: ${fallbackErr}`);
                    }
            ...
            // Nếu fetch thất bại vì lý do mạng, cũng thử fallback
            console.warn(`[${sheetNameForAPI}] Primary fetch failed, trying fallback:`, err);
            try {
                const fallbackUrl = `/data/${sheetNameForAPI}.json`;
                const fallbackRes = await fetch(fallbackUrl);
                if (!fallbackRes.ok) {
    ...
    async function fetchAndCacheAllDataForSheet(sheetNameNormalized, sheetNameForAPI) {
        ...
        try {
            const data = await cacheManager.getDataWithStaleWhileRevalidate(
                cacheKey,
                async () => {
                    // Function để fetch dữ liệu mới (có fallback)
            const result = await fetchFromApiOrPublic(sheetNameNormalized, sheetNameForAPI);
