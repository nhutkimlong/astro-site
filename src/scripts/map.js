...
// Local debounce utility to avoid dependency on other bundles
...
// --- POI Data Handling ---
async function fetchJsonWithFallback(primaryUrl, fallbackPath, contextLabel) {
    try {
        ...
        if (!res.ok) {
            ...
            if (res.status === 404) {
                ...
                if (!fbRes.ok) throw new Error(`[${contextLabel}] Fallback error ${fbRes.status}: ${await fbRes.text()}`);
                return await fbRes.json();
            }
            throw new Error(`[${contextLabel}] HTTP ${res.status}: ${text}`);
        }
...
async function loadPoiData() {
    ...
    try {
        ...
        const data = await cacheManager.getDataWithStaleWhileRevalidate(
            ...
            async () => {
                // Fetch từ Blobs, fallback static khi 404/ lỗi mạng
                const data = await fetchJsonWithFallback(LOCAL_DATA_API_POI, '/data/POI.json', 'POI');
                return data.map(poi => {
...
async function loadGioHoatDong() {
    try {
        ...
        gioHoatDongData = await fetchJsonWithFallback(LOCAL_DATA_API_GIO, '/data/GioHoatDong.json', 'GioHoatDong');
